...
public class MatriculaServiceImpl implements IMatriculaService {
	Logger LOGGER = LoggerFactory.getLogger(MatriculaServiceImpl.class);
	@Autowired
	...
	public List<Matricula> findAll() {
		...
		matriculas = matriculas.stream().map(item -> {
			Alumno alumno = alumnoClient.findAlumno(item.getAlumnoId());
	...
	public Matricula findById(int id) {
		...
		if (matricula != null) {
			Alumno alumno = alumnoClient.findAlumno(matricula.getAlumnoId());
